...
#define NS_DRAM0_SIZE			ULL(0x200000000) /* 8GB */
...
#define PLAT_PHY_ADDR_SPACE_SIZE	(1ULL << 38)
#define PLAT_VIRT_ADDR_SPACE_SIZE	(1ULL << 38)
...
/* UART0 is for non-secure world
 * UART1 is reserved for secure in DTB, it's the only one we can use
 */
#define PLAT_RSE_SERIAL_UART_BASE	UART1_BASE
#define PLAT_RSE_SERIAL_UART_CLK_IN_HZ	UART1_CLK_IN_HZ
...
#define PLAT_QEMU_L1_GPT_SIZE		UL(0x00200000)	/* 2MB */
...
/*
 * Need to be in sync with RMM's RMM_MAX_SIZE variable in
 * qemu_virt_defcfg.cmake
 */
#define REALM_DRAM_SIZE			0x02800000
